...
export function LogTab({ sessionId }: LogTabProps) {
  ...
  const loadLogs = useCallback(async () => {
    setLoading(true)
    try {
      const logs = await logger.getCalculationLogs()
  ...
  const clearAllLogs = async () => {
    if (confirm("Are you sure you want to clear all calculation logs? This action cannot be undone.")) {
      await logger.clearCalculationLogs()
      setCalculationLogs([])
    }
  }
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              When signed in, your calculation logs are saved to the cloud and synced across devices.
              When not signed in, logs are stored locally in your browser and will be lost if you clear your browser storage.
            </p>
          </CardContent>
        </Card>
      </div>
